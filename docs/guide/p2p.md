...
只要双方持有相同的 **Session Key (会话密钥)**，`gonc` 就会自动完成信令交换、NAT 打洞和连接建立。如果双方都内一个内网，可以双方都加入`-lan`参数以便更快建立连接。
...
该示例仅展示了基础的 P2P 连接能力。如需构建可复用的内网穿透隧道，可结合使用支持多路复用的内置模块[linkagent](../cookbook.md#link)。
...
| **`-lan`** | **内网环境 P2P 模式**。在内网基于口令发现彼此并绕过防火墙建立连接，不依赖MQTT和STUN。 |
